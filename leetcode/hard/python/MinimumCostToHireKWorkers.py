...
# using tuple and min values for the heap to make it act like max heap
class Solution:
    def mincostToHireWorkers(self, quality: List[int], wage: List[int], k: int) -> float:

        workers = sorted([(q, w, w / q) for q, w in zip(quality, wage)], key=lambda x: x[2])
        ...
        for q, w, ratio in workers:
            # Push the negative of quality to make Python's min heap act like a max heap
            heapq.heappush(max_heap, -q)
            quality_sum += q
            
            if len(max_heap) > k:
                quality_sum += heapq.heappop(max_heap)    # Remove the largest quality to maintain size k, add back as we pushed negative values
